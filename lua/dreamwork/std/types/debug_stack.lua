...
--- [SHARED AND MENU]
---
--- Initializes the stack, resetting it to be empty.
---
---@protected
...
--- workspace-relative path, so stored frames stay readable regardless of
--- how the source path was originally reported (e.g. with a leading `@`).
---
--- C frames (`source == "=[C]"`) and frames without a source are left
--- untouched.
---
...
--- in bottom-to-top order. On subsequent calls, the newly captured frames
--- are compared against the frames already stored, starting at the top of
--- the stack, and matched by `currentline`, `source` and `name`. Frames
--- behind the first match are assumed unchanged and are left alone; only
--- the new frames above the match (i.e. calls made since the previous
--- capture) are appended. If no match is found, the new capture is
--- appended wholesale on top of the existing frames.
---
--- Every stored frame has its `source` normalized via `update_source`
--- before being written into the stack.
---
...
function Stack:capture( stack_level, head_skip, tail_skip, max_levels )
    ...
    if stack_size == 0 then
        for i = 1, level_count, 1 do
            self[ i ] = update_source( levels[ (level_count - i) + 1 ] )
        end
    ...
    for d = 0, math_min( (stack_size - self.start) + 1, level_count ) - 1, 1 do
        ...
        if old_info.currentline == new_info.currentline
            ...
            for i = 1, d, 1 do
                self[ (stack_size - d) + i ] = update_source( levels[ (d - i) + 1 ] )
            end
    ...
    for i = 1, level_count, 1 do
        self[ (start + i) - 1 ] = update_source( levels[ (level_count - i) + 1 ] )
    end
